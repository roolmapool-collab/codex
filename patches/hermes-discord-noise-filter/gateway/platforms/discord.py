...
_FORBIDDEN_DISCORD_TEXT_MENTION_PATTERN = re.compile(
...
def _assert_discord_text_mentions_safe(content: Optional[str]) -> None:
    """Block textual agent mentions; Discord agent mentions must be numeric."""
    if not content:
        return
    match = _FORBIDDEN_DISCORD_TEXT_MENTION_PATTERN.search(content)
    if match:
        raise ValueError(
            f"FORBIDDEN_TEXT_MENTION: use numeric Discord mention instead of {match.group(0)!r}"
        )
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        try:
            ...
            if content is None:
                return SendResult(
                    success=True,
                    raw_response={"skipped": True, "reason": "discord_noise_suppressed"},
                )
            _assert_discord_text_mentions_safe(content)
    ...
    async def _forum_post_file(
        ...
        *,
        ...
        content = _sanitize_discord_outbound_content(content) or ""
        if not thread_name:
    ...
    async def _send_file_attachment(
        ...
        caption = _sanitize_discord_outbound_content(caption)
        try:
            _assert_discord_text_mentions_safe(caption)
        except ValueError as e:
            return SendResult(success=False, error=str(e))
    ...
    async def send_multiple_images(
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                content = _sanitize_discord_outbound_content(captions[0]) if captions else None
                try:
                    _assert_discord_text_mentions_safe(content)
                except ValueError as e:
                    logger.warning("[%s] Blocked multi-image Discord send: %s", self.name, e)
                    continue
                logger.info(
