...
_FORBIDDEN_DISCORD_TEXT_MENTION_RE = re.compile(
    r"(?<![\w.])@(?:Hermes|Ariane|Vulcain|Argus|Atlas)\b|<@!?(?!\d+>)[^>]+>",
    re.IGNORECASE,
)
_DISCORD_NUMERIC_MENTION_RE = re.compile(r"<@!?(\d+)>")
_DISCORD_OUTBOUND_NOISE_LINE_PATTERNS = (
    re.compile(r"^\s*(?:💻\s*)?terminal\s*:\s*\".*\"\s*$", re.IGNORECASE),
    re.compile(r"^\s*(?:🔎\s*)?search_files\s*:\s*\".*\"\s*$", re.IGNORECASE),
    re.compile(r"^\s*(?:📖\s*)?read_file\s*:\s*\".*\"\s*$", re.IGNORECASE),
    re.compile(r"^\s*(?:🛠️|🔧|🧰)\s*[A-Za-z0-9_-]+\s*:\s*\".*\"\s*$", re.IGNORECASE),
    re.compile(r".*\bType\s+/sethome\s+to make this chat your home channel\b.*", re.IGNORECASE),
    re.compile(r".*\bA home channel is where Hermes delivers cron job results\b.*", re.IGNORECASE),
    re.compile(r".*\bNo home channel is set\b.*", re.IGNORECASE),
    re.compile(r"^\s*Je dois d.?abord vérifier\b.*$", re.IGNORECASE),
)


def _sanitize_discord_outbound_content(content: Optional[str]) -> Optional[str]:
    """Strip gateway/tool chatter from Discord outbound text."""
    if content is None:
        return None
    kept = []
    for line in str(content).splitlines():
        if any(pattern.search(line) for pattern in _DISCORD_OUTBOUND_NOISE_LINE_PATTERNS):
            continue
        kept.append(line)
    sanitized = "\n".join(kept).strip()
...
def _validate_discord_message_text(message: str) -> Optional[str]:
    """Reject textual agent mentions before any Discord REST send."""
    match = _FORBIDDEN_DISCORD_TEXT_MENTION_RE.search(message or "")
    if not match:
        return None
    return f"FORBIDDEN_TEXT_MENTION: use numeric Discord mention instead of {match.group(0)!r}"


def _discord_message_payload(message: str) -> dict:
    """Build a Discord payload with explicit, scoped allowed_mentions."""
    users = sorted(set(_DISCORD_NUMERIC_MENTION_RE.findall(message or "")))
    return {
        "content": message,
        "allowed_mentions": {
            "parse": [],
...
def _handle_send(args):
    ...
    if platform_name == "discord":
        message = _sanitize_discord_outbound_content(message)
        if message is None:
            return json.dumps({
                "success": True,
                "platform": "discord",
                "skipped": True,
                "reason": "discord_noise_suppressed",
            })
        validation_error = _validate_discord_message_text(message)
        if validation_error:
            return json.dumps(_error(validation_error))
            ...
            if is_forum:
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60), **_sess_kw) as session:
                    if valid_media:
                        ...
                        starter_message = {
                            **_discord_message_payload(message),
